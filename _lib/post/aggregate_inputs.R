...
aggregate_input_data <- function(reg,
  ...
  if (build | !(output_file %>% file.exists)) {
    ...
    a0_shp <- readRDS(paste0(file_dir,'lbd_standard_admin_0.rds')) %>% st_as_sf
    a1_shp <- readRDS(paste0(file_dir,'lbd_standard_admin_1.rds')) %>% st_as_sf
    ...
    input_sf <- st_as_sf(input_data, coords = c("longitude", "latitude"), crs=st_crs(a0_shp), agr = "constant")

    #extract admin0 ids using sf 
    #use st_nearest to give the best match possible for points that are misaligned
    ints <- sf::st_nearest_feature(input_sf, a1_shp)
    input_sf$ADM0_CODE <- a1_shp[ints %>% unlist,]$ADM0_CODE %>% as.character %>% as.integer
    input_sf$ADM1_CODE <- a1_shp[ints %>% unlist,]$ADM1_CODE %>% as.character %>% as.integer
    input_data <- input_sf %>% 
      st_drop_geometry %>% 
      as.data.table
    
    #reset weights for point data
    input_data[point==TRUE, weight := raw_weight]
    
    # Summarize input data by admin 0 IDs
    a0_input <- copy(input_data) %>% 
      setkeyv(., c('nid', 'year', 'ADM0_CODE')) %>% 
      .[, input_mean := weighted.mean(prop, raw_weight), by = key(.)] %>% 
      .[, input_ss := sum(raw_weight*N), by = key(.)] %>% 
      unique(., by=key(.)) %>% 
...
aggregate_child_stackers <- function(reg,
  ...
  # ---------------------------------------------------------------------------------------------------------------
    ...
    # ----------------------------------------------------------------------------------------
    # Add in population
    pop <- load_worldpop_covariate(template_raster = simple_raster,
