...
load_map_results <- function(indicator, indicator_group, run_date, map_region, type, raked, start_year, end_year, 
  ...
  if ("admin0" %in% geo_levels) {
    ...
    if(missing(custom_data)) {
      pred <- paste0(maps_path, "/pred_derivatives/admin_summaries/", indicator, "_admin_0_", 
                     ifelse(raked, "raked", "unraked"), ifelse(is.null(raked_measure), '', paste0('_', raked_measure)), "_summary.csv") %>% fread
    } else {
