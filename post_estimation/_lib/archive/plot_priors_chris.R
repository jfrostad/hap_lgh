...
plot_spatial_priors <- function(indicator,
  ...
  library(MASS)
  library(ggplot2)
  library(actuar, lib.loc = '/homes/kewiens/libraries/')
  library(mvtnorm)
  library(data.table)
  
  
  ### Load a fitted INLA model object
  fit <- readRDS(paste0('/share/geospatial/mbg/', indicator_group, '/', indicator, '/output/', run_date, '/inla_model_fit_pre_preds_', region, '_holdout_', holdout, '_agebin_', age, '.RDS'))
  
  ### Blangiardo and Cameletti (2015): pg. 197: "In R-INLA the default internal representation for the SPDE parameters is log(Tau) = Theta1 and
  ### log(Kappa) = Theta2, with Theta1 and Theta2 being given a joint Normal prior distribution (by default independent)."
  ### pg. 196: For Alpha = 2 (default smoothness in INLA): r = range = sqrt(8)/Kappa ; variance = 1/(4*pi*Kappa^2*Tau^2)
  
  ### See https://groups.google.com/forum/#!searchin/r-inla-discussion-group/spde$20$20default$20prior/r-inla-discussion-group/gIGkLLqCtpU/oppbQb5lb-4J
  ### See https://groups.google.com/forum/embed/?parenturl=http%3A%2F%2Fwww.r-inla.org%2Fcomments-1&service=jotspot&ul=1&theme=default&place=forum%2Fr-inla-discussion-group&showpopout=true&showsearch=true#!searchin/r-inla-discussion-group/get$20priors/r-inla-discussion-group/eqMhlbwChkQ/16cNkIx12xUJ
  ...
  #colnames(posteriors) <- c("Theta1.Posterior", "Theta2.Posterior", "Rho.Posterior", "IID.ID.Precision.Posterior", "CTRY.ID.Precision.Posterior")
  ...
  ### Save plots
  savedir <- paste0('/share/geospatial/mbg/', indicator_group, '/', indicator, '/output/', run_date, '/diagnostic_plots/')
